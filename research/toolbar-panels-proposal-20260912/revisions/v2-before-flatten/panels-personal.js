...
function renderExtensions(ctx) {
  ...
  return `<div class="panel-body">${intro('扩展按各自内容展开；下面保留原生的两列折叠结构。')}
    <div class="list-toolbar">${toggle(ctx, 'extensions', 'notify', '在扩展程序更新时通知')}<div class="actions">${actionButton(ctx, '管理扩展程序', 'personal-extensions-manage', { icon: 'cubes' })}${actionButton(ctx, '安装扩展程序', 'personal-extensions-install', { icon: 'import', kind: 'primary' })}</div></div>
    <p class="status-line"><span class="tag">${activeCount} 个已启用</span><span class="muted">示例状态，仅在本页生效</span></p>
