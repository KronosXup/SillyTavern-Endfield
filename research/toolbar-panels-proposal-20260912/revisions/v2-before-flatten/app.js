...
function arrangePanelTools() {
  ...
  for(const el of [...content.children]) {
    ...
    else if(activePanel==='extensions'&&el.matches('.status-line'))selected.push(el);
  }
  if(selected.length){
  ...
  if(activePanel==='personas') {
    ...
    if(list){const tools=document.createElement('div');tools.className='resource-tools';
      [...list.children].filter(el=>el.matches('.list-toolbar,.search-row')).forEach(el=>tools.append(el));list.prepend(tools);}
  }
}
function render({resetScroll=false}={}) {
  ...
  document.title=`Endfield · ${p.label} · 七面板 V2`;
}
