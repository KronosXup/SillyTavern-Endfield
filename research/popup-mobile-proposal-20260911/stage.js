...
const cases = {
  ...
  'input-reference': { text: '请输入好友的备注名', input: '', placeholder: '请输入好友的备注名', hint: '好友备注名不能超过7个汉字（15个字符），且不能包含特殊字符', ok: '确认', cancel: '取消' },
  ...
  'checkbox-reference': { html: '<h3>当前理智不足，无法领取奖励，是否继续进入协议空间？</h3><p class="opacity50p">在领取奖励时，可以使用理智道具或源石恢复理智。</p>', checkbox: '今日不再提醒', ok: '确认', cancel: '取消' },
...
if (specimen.placeholder) input.placeholder = specimen.placeholder;
if (specimen.hint) {
  inputs.style.display = 'block';
  const hint = document.createElement('p'); hint.className = 'popup-input-hint'; hint.id = 'popup-input-hint';
  hint.textContent = specimen.hint; inputs.append(hint); input.setAttribute('aria-describedby', hint.id);
}
